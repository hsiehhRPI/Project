...
void deleteAll(Balloon* b) {
  std::set<Balloon*> holder;
  holder[0] = b;
  for (int n = 0; n < b.ropes.size(); n++) {
    holder[n+holder.size()] = b.getRope[n];
  }
  for (std::set<Balloon*>::iterator y = holder.begin(); y != holder.end(); ++y) {
    std::set<Balloon*>::iterator tmp = y;
    ++tmp;
  }
}
