...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    unsigned int hashV = m_hash(key);
    unsigned int idx = hashV % m_table.size();
    hash_list_itr p = std::find(m_table[idx].begin(), m_table[idx].end(), key);
    //If element already exist in the hashset
    if (p != m_table[idx].end()) {
      result.first = iterator(this, idx, p);
      result.second = false;
      return result;
    }
    //If element does not exist in the hashset
    if (p == m_table[idx].end()) {
      //Reset p to the begining of the list inside the set
  ...
  iterator begin() {
    ...
    for (int n = 0; n < m_size; n++) {
      if (m_table[n].size() != 0) {
        hash_list_itr b= m_table[n].begin();
...
private:
  ...
  void resize_table(unsigned int new_size) {
    ...
    //Make a completely empty new hashset first
    std::vector<std::list<KeyType>> t_table;
    for (int n = 0; n < new_size; n++) {
      std::list<KeyType> lST;
      t_table.push_back(lST);
    }

    //Reassigning old value
    for (int u = 0; u < m_table.size(); u++) {
      if (m_table[u].size() > 0) {
        for (hash_list_itr y = m_table[u].begin(); y != m_table[u].end(); ++y) {
          unsigned int hash_value = m_hash(*y);
          unsigned int index = hash_value % t_table.size();
          t_table[index].push_back(*y);
        }
      }
    }
     m_table = t_table;
