...
//This following block is notes for class discussion on Friday
...
int main(int argc, char* argv []) {
	...
	if (!invFile.good() || !cusFile.good()) {
		...
		//I really don't know how to account for multiple events for the same person
		//I kept all the events into customer objects despite knowing there are duplicates so we could step through them
		invFile.close();
	...
	while(exeIndex < storage.size()) {
		while (lIndex < customers.size()) {
			...
			if (t_itr->tGetID() == c_itr->cGetTID() && t_itr->tGetQuan() >= c_itr->cGetQuant()) {
				//If there is enough tool for the customer to rent
				if (c_itr->cGetStat() == "rent") {
					t_itr->tSet_quan(t_itr->tGetQuan()-c_itr->cGetQuant());
					t_itr->tAdd_rcid(c_itr->cGetCID());
					t_itr->tAdd_cName(c_itr->cGetName());
					t_itr->tAdd_rq(c_itr->cGetQuant());
					c_itr->cSetState("rent");
					++c_itr;
					lIndex++;
				}
			}
	...
	//This part 
	std::list<Tool>::iterator tcomp = storage.begin();
	for (t_itr = storage.begin(); t_itr != storage.end(); ++t_itr) {
		for (tcomp = storage.begin(); tcomp!=storage.end(); ++tcomp) {
			if (t_itr->tGetID() < tcomp->tGetID()) {
				std::swap(*t_itr, *tcomp);
			}
		}
	}
	std::list<Customer>::iterator ccomp = customers.begin();
	for (c_itr = customers.begin(); c_itr != customers.end(); ++c_itr) {
		for (ccomp = customers.begin(); ccomp!=customers.end(); ++ccomp) {
			if (c_itr->cGetCID() < ccomp->cGetCID()) {
				std::swap(*c_itr, *ccomp);
			}
		}
	}
	//There are duplicates in the customer file, so we will have to get rid of them
	//(for simplicity, I'm gonna move these into another list)
	std::list<Customer> uniqueC;
	std::list<Customer>::iterator next = customers.begin();
	c_itr = customers.begin();
	++next;
	while(next!=customers.end()) {
		if (c_itr->cGetCID()!=next->cGetCID()){
			uniqueC.push_back(*c_itr);
		}
		else if (c_itr->cGetCID()==next->cGetCID()){
			if (c_itr->cGetStat() == "rent" && c_itr->cGetStat() == next->cGetStat()) {
				uniqueC.back().cSetQuan(uniqueC.back().cGetQuant()+next->cGetQuant());
			}
		}
		++c_itr;
		++next;
	}
	//Since all the pending status is only finalized until every event in the list is done
	//We need to update the pending category for each of the tools
	t_itr = storage.begin();
	for (int u = 0; u < storage.size(); u++) {
		for (c_itr = uniqueC.begin(); c_itr!=uniqueC.end(); ++c_itr) {
			if (t_itr->tGetID() == c_itr->cGetTID() && c_itr->cGetStat() == "pending") {
			t_itr->tAdd_pq(c_itr->cGetQuant());
			t_itr->tAdd_pName(c_itr->cGetName());
			t_itr->tAdd_pcid(c_itr->cGetCID());
			}
		}
		++t_itr;
	}
	...
	std::ofstream invOut (invFileOut);
	t_itr = storage.begin();
	for (long unsigned int v = 0; v < storage.size(); v++) {
		//I did not account for multiple events from the same person so I'm trying my best to fix this
		invOut << *t_itr << std::endl;
	...
	c_itr = uniqueC.begin();
	for (long unsigned int j = 0; j < uniqueC.size(); j++) {
		//I did not account for multiple events from the same person so I'm trying my best to fix this
		cusOut << *c_itr;
		++c_itr;
	}
