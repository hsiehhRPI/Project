...
int main(int argc, char* argv []) {
	...
	for (int e = 0; e < eachLine.size(); e++) {
		if (e>0==true) {
			...
			else if (eachLine[e].find("._") != std::string::npos) {
				continue;
			}
		...
		for (int r = 0; r < schl_player.size(); r++) {
			if (r > 0 == true) {
				if (athlete[index].getName() == schl_player[r]) {
					...
					else if(schl_player[r-1].find("_") != std::string::npos) {
	...
	std::vector<std::string> teamPenal;
		...
		while (pIndex < uni.size()) {
			for (int d = 0; d < teamPenal.size(); d++) {
				if (teamPenal[d] == uni[pIndex].getSName()) {
					if (teamPenal[d+2] == "2:00") {
						uni[pIndex].set_PenTime(uni[pIndex].getPenTime()+2);
					}
					else if (teamPenal[d+2] == "5:00") {
						uni[pIndex].set_PenTime(uni[pIndex].getPenTime()+5);
					}
					else if (teamPenal[d+1] == "10:00") {
						uni[pIndex].set_PenTime(uni[pIndex].getPenTime()+10);
					}
				}
			}
			pIndex ++;
	...
	if (status == "--team_stats") {
		...
		for (int h = 0; h < uni.size(); h++) {
			outfile << uni[h];
			/*outfile << std::setw(maxSname+2)<< std::left << std::string(uni[h].getSName());
			outfile << std::setw(3)<< std::right << std::string(uni[h].getSWin());
			outfile << std::setw(4)<< std::right << std::string(uni[h].getSLose());
			outfile << std::setw(5)<< std::right << std::string(uni[h].getSTies());
			outfile << std::setw(6)<< std::right << std::setprecision(2) << std::string(uni[h].getSWinRate());
			outfile << std::setw(8)<< std::right << std::string(uni[h].getSGoal());
			outfile << std::setw(11)<< std::right << std::string(uni[h].getSPenalty());*/
		}
		outfile.close();
	}
		...
		for (int h = 0; h < athlete.size(); h++) {
			outfile << athlete[h];
			/*
			std::cout.width(maxName+2);
			std::cout<<std::left << athlete[h].getName();
			std::cout.width(maxSname+3);
			std::cout << std::left << athlete[h].getUni();
			std::cout.width(5);
			std::cout << std::right << athlete[h].getGoal();
			std::cout.width(9);
			std::cout << std::right << athlete[h].getAssist();
			std::cout.width(11);
			std::cout << std::right << athlete[h].getPenal() <<std::endl;
			*/
		}
		outfile.close();
}
...
/*
	else if (status == "--custom_stats") {
		std::ofstream outfile (argv[2]);
		outfile << "The total time out in this season is: " << (std::string)(totalTimeout/60)<<" hours and "<< (std::string)(totalTimeout%60) <<" minutes"<<std::endl;
		std::cout.width(10 + 3);
		std::cout<<std::left <<"Team Name";
		std::cout.width(12 + 3);
		std::cout<<std::left <<"Penalty Time"<<std::endl;
		for (int n = 0; n < uni.size(); n++) {
			std::cout.width(10 + 3);
			std::cout<<std::left <<uni[n].getSName();
			std::cout.width(7);
			std::cout << std::fixed;
			std::cout<<std::right << std::setprecision(2) << uni[n].getPenTime()/60 <<"h ";
			std::cout.width(2);
			std::cout<<std::setprecision(2) << uni[n].getPenTime()%60 <<"m"<<std::endl;
		}
		//Getting the rarest penalty
		std::cout << "The least likely penalty is: "<< tPenal[pos] <<", which happened "<< minimum<<" time(s)."<<std::endl;
		outfile.close();
	}
	*/
