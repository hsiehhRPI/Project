...
/*
  TrainCar* train = NULL;
  PushBack(train, TrainCar::MakeEngine()); 
  PushBack(train, TrainCar::MakePassengerCar());
  PushBack(train, TrainCar::MakeEngine()); 

  // inspect the cars, the links, the links, and sequential IDs...
  assert (train->isEngine());
  assert (train->prev == NULL);
  assert (train->next->isPassengerCar());
  assert (train->next->prev->isEngine());
  assert (train->next->next->isEngine());
  // helper routine sanity check & print the results
  TrainCar*t2;
  TrainCar*t3;
  SeparateTestHelper(train, "#1");
  SanityCheck(train);
  PrintTrain(train);

  std::cout << "ship test" << std::endl;
  ShipFreightHelper(mtrand_fixed_seed,10,25,60,12);
